...
def add_shift
	if (params[:date]  != ("" || nil)) && (params[:time1] != ("" || nil) ) && (params[:time2] != ("" || nil) ) && (params[:breaklength] != ("" || nil) )
		...
		Shift.create(user_id: current_user.id, date: params[:date], start: params[:time1], finish: params[:time2], breaklength: params[:breaklength], org_id: current_user.org_id)
	else
	end
